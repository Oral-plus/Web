<?php
// Conectar a la base de datos de SQL Server
$serverName = "EASYNEWS\\SQLEXPRESS";
$connectionOptions = array(
    "Database" => "calidad",
    "Uid" => "sa",
    "PWD" => "Omega"
);
$conn = sqlsrv_connect($serverName, $connectionOptions);

if (!$conn) {
    die("Error al conectar a la base de datos: " . sqlsrv_errors());
}

// Obtener los valores de los campos
$id = $_POST['id'];
$nombre_producto = $_POST['nombre_producto'];
$vendedor = $_POST['vendedor'];
$kam = $_POST['kam'];
$ciudad = $_POST['ciudad'];
$coach = $_POST['coach'];
$fecha = $_POST['fecha'];

// Comprobar si se ha enviado una imagen
if (isset($_FILES['imagen']) && $_FILES['imagen']['error'] === UPLOAD_ERR_OK) {
    $nombreImagen = $_FILES['imagen']['name'];
    $tipoImagen = $_FILES['imagen']['type'];
    $datosImagen = file_get_contents($_FILES['imagen']['tmp_name']);

    // Insertar la imagen y los datos en la base de datos
    $sql = "INSERT INTO imagenes (nombre, tipo, datos, id, nombre_producto, vendedor, kam, ciudad, coach, fecha) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $params = array($nombreImagen, $tipoImagen, $datosImagen, $id, $nombre_producto, $vendedor, $kam, $ciudad, $coach, $fecha);
    $stmt = sqlsrv_query($conn, $sql, $params);

    if ($stmt) {
        echo "La imagen y los datos se han subido correctamente.";
    } else {
        echo "Error al subir la imagen y los datos: " . sqlsrv_errors();
    }

    sqlsrv_free_stmt($stmt);
} else {
    echo "Error al subir la imagen.";
}

// Cerrar la conexión
sqlsrv_close($conn);
?>
