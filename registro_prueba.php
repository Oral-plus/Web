...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        ...
        $sql = "SELECT * FROM CONSULTA_CARTERA3 WHERE CardCode = ?";
            ...
            <table class="table">
                <thead class="table-success table-striped">
                    <tr>
                        ...
                        <th>FACTURA</th>
                        <th>FECHA FACTURA</th>
                        <th>EDAD DE LAS FACTURAS EN DIA</th>
                        <th>VALOR</th>
                ...
                <tbody>
                    ...
                    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        $dias = $row['dias'];
                        $precio = $row['valor_formateado'] * 100; 
                        $descuento2 =  $row['valor_formateado'] * 100; 

                        // Asegúrate de que 'dias' sea un número
                        if(is_numeric($dias)){
                            if($dias >1 && $dias <= 15){
                                $precio = ($precio - ($precio * 2.5 / 100));
                                $precio = intval($precio);  // Convertir a entero
                            }elseif ($dias >=16 && $dias <= 30) {
                                $precio = ($precio-($precio * 1.5 /100));
                                $precio = intval($precio);  // Convertir a entero
                            }
                            } 
                            ...
                            if(is_numeric($dias)){
                                if($dias >= 1 && $dias <= 15){
                                    $descuento2 = (($descuento2 * 0.025) /100);
                                }elseif ($dias >=16 && $dias <= 30) {
                                    $descuento2 = (($descuento2 * 0.015 /100));
                                }elseif ($dias >=31) {
                                    $descuento2 = (($descuento2 * 0));
                                }
                                }
...
$valor_formateado2 = number_format($descuento2, 2, ',', '.'); // Usa ',' para decimales y '.' para miles

// Si quieres incluir el símbolo de la moneda, por ejemplo, "$"
$valor_con_simbolo2 = '$' . $valor_formateado2;
                        ?>
                        <tr>
                            ...
                            <td><?php echo htmlspecialchars($row['DocDate'], ENT_QUOTES, 'UTF-8') ?></td>
                            <td><?php echo htmlspecialchars($row['dias'], ENT_QUOTES, 'UTF-8') ?></td>
                            <td><?php echo htmlspecialchars($valor_con_simbolo, ENT_QUOTES, 'UTF-8') ?></td>
                            <td><?php echo htmlspecialchars($valor_con_simbolo2, ENT_QUOTES, 'UTF-8') ?></td>
